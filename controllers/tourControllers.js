...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  ...
  // Save cover image as-is (no resizing)
  const coverFile = req.files.imageCover[0];
  const coverExt = path.extname(coverFile.originalname);
  const coverFilename = `tour-${req.params.id}-${Date.now()}-cover${coverExt}`;
...
// Aggregation pipelines and other handlers remain unchanged
